...
// WHY O(m+n)?
// each element is visited exactly once — single pass from back to front

// Core idea: fill nums1 from the back using 3 pointers
// p1 → last real element of nums1
// p2 → last element of nums2
// p3 → last position to fill in nums1
// compare nums1[p1] vs nums2[p2] → place larger at p3 → move that pointer back

// WHY FROM THE BACK?
// nums1 has empty slots at the end — filling from back avoids shifting elements
// merging from front would cause O(n) shifts per insert → O(n²) total
